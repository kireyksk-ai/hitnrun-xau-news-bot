...
MANDATORY SHARE RULE: Set material=true for every fresh headline with any defensible direct or second-order impact on XAUUSD, gold, crude oil/energy, inflation expectations, energy logistics, US yields, DXY, Fed pricing or risk sentiment. This includes impact levels 1 through 5; use the level only to describe magnitude, never as a reason to suppress a relevant headline. A credible single-source report must be published as BREAKING/UNVERIFIED. A follow-up is not a duplicate when it adds a new fact, official confirmation, escalation, actual data result, revision or visible market reaction. Do not require the headline to mention gold directly when its transmission to USD, yields, liquidity, risk sentiment, energy inflation or gold is defensible. When in doubt, publish with uncertainty stated explicitly. Reject only a truly stale item, an exact or near-exact repeat with no new information, pure clickbait/opinion, or an item with no plausible XAU, oil or inflation transmission channel at all.
