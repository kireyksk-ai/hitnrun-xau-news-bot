...
export type QuantitativeState = { observations:Record<string,QuantObservation>; models:QuantModel[]; relationships:Relationship[]; positioning:PositioningState[]; scorecards:Scorecard[]; sourceEvidence:Record<string,SourceEvidence>; drift:DriftRecord[]; processed?:Record<string,string>; calibration?:Record<string,{evaluated:number;uncertain:number;hits:number;sampleSize:number;state:string;updatedAt:string}>; actorEvidence?:Record<string,{observations:number;useful:number;state:string}> };
