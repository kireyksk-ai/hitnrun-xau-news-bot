...
export async function observeMarket(store: IntelligenceStore): Promise<{ point: MarketPoint; decision: ShadowDecision }> {
  ...
  for(const checkpoint of store.dueCheckpoints(point.capturedAt)) { const completed=complete(checkpoint,point.capturedAt,{xauChange:xauChange(previousXau?.price,values.XAUUSD?.price),dxyChange:xauChange(previousPoint?.values.DXY?.price,values.DXY?.price),yieldChange:xauChange(previousPoint?.values.US10Y?.price,values.US10Y?.price),oilChange:xauChange(previousPoint?.values.WTI?.price,values.WTI?.price),session:point.session},values.XAUUSD?.quality??"DATA_UNAVAILABLE"); store.updateCheckpoint(completed); if(completed.status==="COMPLETED") store.recordExperience({id:`checkpoint:${completed.id}`,createdAt:point.capturedAt,regime:store.regime,trigger:completed.eventId,attribution:completed.outcome==="MARKET_CONFIRMATION"?"POSSIBLE_DRIVER":completed.outcome==="NO_EFFECT"?"INSUFFICIENT_EVIDENCE":"DRIVER_UNKNOWN",confidence:0,outcome:completed.outcome,quantitative:{scorecardId:completed.id}}); }
